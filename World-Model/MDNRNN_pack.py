...
class MDNRNN():
    ...
    def train(self, input_obs, target_obs, input_actions, target_done, target_reward, mask, n_epochs):
        ...
        lenghts = list(map(int,mask.sum(dim=1).cpu().numpy()))
        sorted_index = sorted(range(len(lenghts)), key=lambda k: lenghts[k],reverse=True)
        sorted_lenghts = sorted(lenghts,reverse=True)
            
        inputs = torch.cat((input_obs, input_actions),-1)
        
        pack_inputs = pack_padded_sequence(inputs[sorted_index,:],sorted_lenghts, batch_first=True)
        pack_target_obs = pack_padded_sequence(target_obs[sorted_index,:],sorted_lenghts, batch_first=True)
        pack_target_reward = pack_padded_sequence(target_reward[sorted_index,:],sorted_lenghts, batch_first=True)
        pack_target_done = pack_padded_sequence(target_done[sorted_index,:],sorted_lenghts, batch_first=True)
        
        past_loss = 0
        for epoch in range(n_epochs):
            hidden = self.model.init_hidden(inputs.size(0)).to(self.device)    
            self.optimizer.zero_grad()
            prob_rewards, prob_done, (logpi, mu, logsigma), hidden = self.model(pack_inputs, hidden) 
            loss_mdn = self.loss_loglikelihood(logpi, mu, logsigma, pack_target_obs.data)
            loss_done, loss_reward = self.loss_objectives(prob_rewards, prob_done, pack_target_reward.data, pack_target_done.data)
...
#            print(self.optimizer)
    ...
    def loss_objectives(self, prob_rewards, prob_done, target_reward, target_done):
        mse = nn.MSELoss()
        loss_done = mse(prob_done.squeeze(),target_done)
        loss_reward = mse(prob_rewards,target_reward)
        return loss_done, loss_reward
        
    def loss_loglikelihood(self, logpi, mu, logsigma, y):
        y = y.unsqueeze(1)

...
class ConvVAE():
    ...
    def train(self, batch_img, nb_epochs):
        batch_img = batch_img.to(self.device)
        for epoch in range(1,nb_epochs+1):
            self.vae.train()
            self.optimizer.zero_grad()
            recon_x, mu, logsigma = self.vae(batch_img)
            
            BCE = F.mse_loss(recon_x, batch_img, reduction='sum')
            ...
            if epoch%100==0:
                print(f'Epoch {epoch}: loss = {round(float(loss),4)}') #CHECK THIS LINE, voir s on peut utiliser tdqm
        
        return recon_x
        
    def __call__(self,batch_img):
        return self.vae(batch_img)[0]

    def display_reconstruction(self,batch_img, recon_batch_img, id_img):
        ...
        for channel in range(3):
            ...
            plt.axis('off')
            img = np.transpose(np.array(recon_batch_img[id_img,:,:,:].detach()*255),(1,2,0))
            plt.subplot(4,2,2*(channel+1))
        ...
        plt.axis('off')
        img = np.transpose(np.array(recon_batch_img[id_img,:,:,:].detach()),(1,2,0))
        plt.subplot(4,2,8)
